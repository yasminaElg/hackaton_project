...
Now, you can install `create-react-app` and bootstrap the project in the current folder at the same time with a `yarn` command. `yarn` is a package manager much like `npm`.

```
yarn create react-app hackaton
```

Your React project is now ready to use ! Go to the newly created directory and open webstorm in this directory

```
webstorm .
```

You ca row run the app via the commands `yarn start` it will run a development server. Access it via `http://localhost:3000`
...
You should also implement the design as you go, don't forget to use `Styled Component` as shown before.
...
## Task : Frontend - Improve your spaghett
